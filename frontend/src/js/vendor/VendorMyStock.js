...
export default function MaterialTableDemo(props) {
  ...
  return (
    <div className={classes.root}>
      <Dialog
        onClose={handleClickCloseImageDialog}
          ...
          <MaterialTable
                ...
                setTimeout(() => {
                  ...
                  Axios.get('http://localhost:8080/GetUserProduct', {
                    ...
                    .then(result => {
                      ...
                      var data = []
                      for (
                        var i = query.pageSize * (query.page + 1) - query.pageSize;
                        i <= query.pageSize * (query.page + 1) - 1;
                        i++
                      ) {
            ...
            editable={{
              onRowAdd: newData =>
                new Promise(resolve => {
                  /**********************************************************************************/
                  const photoRef = productPhotoRef.child(
                    email.concat('-').concat(newData.productName)
                  )

                  if (
                    newData.productName &&
                    newData.productName.trim().length > 0 &&
                    newData.productQuantity &&
                    newData.productQuantity.trim().length > 0 &&
                    newData.productQuantity > 0 &&
                    newData.productCategory &&
                    newData.productCategory.trim().length > 0 &&
                    newData.productPrice &&
                    newData.productPrice.trim().length > 0 &&
              ...
              onRowUpdate: (newData, oldData) =>
                new Promise(resolve => {
                  ...
                  setTimeout(() => {
                    ...
                    Axios.post(
                      'http://localhost:8080/EditProduct',
                      {
                        email: email,
                        role: 'Business owner',
                        name: newData.productName,
